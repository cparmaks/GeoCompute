...
def event_handler_main(in_json_str):
    ...
    downlink_should_be_up = (
        required_bgp_sessions_established(options) <= num_up_bgp_sessions
    )
...
needs_hold_down = {str(needs_hold_down)}"
    ...
    for downlink in options.get("down-links"):
        response_actions.append(
            {
                "set-ephemeral-path": {
                    "path": "interface {0} oper-state".format(downlink),
                    "value": oper_state_str,
                }
            }
        )
    ...
    response = {
        ...
        "persistent-data": response_persistent_data,
    }
